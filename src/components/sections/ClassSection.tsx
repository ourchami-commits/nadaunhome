...
export default function ClassSection() {
  ...
  const currentClass = classes[activeTab];
  const currentTrack = currentClass.tracks[activeTrack] ?? currentClass.tracks.regular;
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="p-8 md:p-10">
              ...
              <div className="mb-6">
                ...
                <ul className="flex flex-wrap gap-2">
                  {currentClass.target.map((t, i) => (
                    <li
                      key={i}
                      className="bg-cream text-dark text-sm px-3 py-1.5 rounded-full border border-border"
                    >
                      {t}
                    </li>
                  ))}
              ...
              {/* Track toggle — only if challenge exists */}
              {hasChallenge && (
                    ...
                    <ul className="space-y-2">
                      {currentTrack.curriculum.map((item, i) => (
                        <li key={i} className="flex items-start gap-3 text-sm">
                          <span className="text-primary font-semibold min-w-[56px] pt-0.5">
                            {item.week}
                          </span>
